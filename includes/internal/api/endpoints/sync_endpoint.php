...
class Sync_Endpoint {
    ...
    public static function register_routes( string $namespace ): void {
        register_rest_route(
            ...
            array(
                ...
                'callback'            => array( self::class, 'handle_sync_product' ),
    ...
    public static function handle_sync_product( WP_REST_Request $request ): WP_REST_Response|WP_Error {
        ...
        if ( empty( $site ) ) {
            return new WP_REST_Response( array( 'message' => 'Can not sync because site does no exist on this page' . $domain ), 404 );
        }
        ...
        return new WP_REST_Response( array( 'product_id' => $result ), 200 );
    }
